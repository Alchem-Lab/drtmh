...
class EpochManager : public RWorker {
 public:
  ...
  uint64_t get_current_timestamp(char *buffer) {
    memcpy(buffer,fetched_ts_buffer_,ts_size_);
  }

  void timestamp_get_handler(int id,int cid,char *msg,void *temp) {
    uint64_t *ts_buffer = (uint64_t *)((char *)(cm_->conn_buf_) + ts_addr_);
    char *reply = rpc_->get_reply_buf();

    uint64_t ts = __sync_fetch_and_add(ts_buffer,1);

    memcpy(reply,&ts,ts_size_);
