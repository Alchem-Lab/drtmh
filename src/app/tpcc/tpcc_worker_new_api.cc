...
txn_result_t TpccWorker::txn_new_order_new_api(calvin_request *req, yield_func_t &yield) {
  ...
  // the execution logic is minimal compared to the computation used to
  // generate the rwsets. 
...
#define MAX_ITEM 15
  ...
  uint64_t no_key = makeNewOrderKey(warehouse_id, districtID, my_next_o_id);
  new_order::value v_no;
  idx = rtx_->insert<NEWO,new_order::value>(current_partition,no_key,&v_no,yield);
  assert (idx == 1);
  ...
  v_oo.o_all_local = allLocal;
  v_oo.o_entry_d = GetCurrentTimeMillis();
  idx = rtx_->insert<ORDE,oorder::value>(current_partition,o_key,(&v_oo),yield); // TODO!!
  assert (idx == 2);
  ...
  array_dummy[0] = 1;
  array_dummy[1] = o_key;
  idx = rtx_->insert<ORDER_INDEX,order_index_type_t>(current_partition,o_sec,(order_index_type_t *)array_dummy,yield);
  assert (idx == 3);

  for (uint ol_number = 1; ol_number <= num_local_stocks; ol_number++) {
    const uint ol_i_id = local_item_ids[ol_number - 1];
    idx = rtx_->read<ITEM,item::value>(current_partition,ol_i_id,yield);
    assert (idx >= 0);
    item::value *i_value = rtx_->get_readset<item::value>(idx,yield);
    ...
    v_ol.ol_supply_w_id = int32_t(local_supplies[ol_number - 1]);
    v_ol.ol_quantity = int8_t(ol_quantity);
    idx = rtx_->insert<ORLI,order_line::value>(current_partition,ol_key,(&v_ol),yield);
    assert (idx >= 0);
  }
