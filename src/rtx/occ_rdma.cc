  ...
  for(auto it = write_set_.begin();it != write_set_.end();++it) {
    if((*it).pid != node_id_) { // remote case
      ...
      DSLR::Lock l = {
        ...
        .local_buf = local_buf,
        .mode = DSLR::EXCLUSIVE,
        .consecutive_failure_times = 0, // the number of consecutive deadlocks/timeouts for the current lock object.
        .resetFrom = 0,
        .elapsed = 0, // this field needs to be set to the time when the lock is actually acquired.
      };
