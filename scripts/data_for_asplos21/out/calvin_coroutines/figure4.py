...
for j,appname in enumerate(apps):
    for i, version in enumerate(versions):
        ...
        for which, alg in enumerate(algs):
            for x in range(datapointnum):
                ...
                val = []
                path_prefix = subprocess.check_output(['ls -d eval_coroutines_calvin*'], shell=True).decode('utf-8').split('\n')[0:-1]
                for trial in range(len(path_prefix)):
                    # print(path_prefix[trial])
                    val.append(get_tput(path_prefix[trial] + '/' + fname))
                print(val)
                num[which].append(max(val))
        rects_list=[]
    ...
    if j == 1:
        plt.legend(ncol=1, bbox_to_anchor=(1, 0.7, 0,0),fontsize=20)
