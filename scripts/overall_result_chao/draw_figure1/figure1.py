...
def set_title(app_idx, num, plt):
    if app_idx == 0 and num % 3 == 0:
        plt.title("Throughput (M txns/s)", fontsize=16, loc='center')
    elif app_idx == 0 and num % 3 == 1:
        plt.title("Latency (ms)", fontsize=16, loc='center')
    elif app_idx == 0:
        plt.title("Abort Rate", fontsize=16, loc='center')
...
plt.legend((rects_list[0][0], rects_list[1][0]), versions, fontsize=16, bbox_to_anchor=(-2.2, -0.9, 1, .06), loc=3, ncol=2,  borderaxespad=0.)
